...
ツリーは多親構造の**射影**であって、正本ではない。

- `workspaceToProjectData` は同じタスクを 1 箇所にしか描かない。同じ id の行が 2 つあると Svelte の keyed each が壊れる（`each_key_duplicate`）ため、意図的に 1 回だけ展開している
- したがって**木の位置だけから `parents` を作り直してはいけない**。描かれなかった辺がファイルからも消え、種別を変えただけ・名前を直しただけの保存でも親が削られる（実際にそうなっていた）
- `projectDataToWorkspaceTasks` の `resolveParents` は、木が見せている親が既知の親に含まれていれば「構造は動いていない」と判断して既知の親を残す。木の位置が既知に無い親へ変わったときだけ、移動とみなして木の位置を採る
- この近似は「構造を触っていない保存が壊さない」ことだけを保証する。**多親ノードを木の上で動かすと、他の親は落ちる**。曖昧さは行をノードではなく辺として扱う（同じノードを親ごとに描く）ようにして初めて解消する
