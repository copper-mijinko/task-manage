...
SVG diff だけでは「改善なのか劣化なのか」は判断できないため、radar-beta / block-beta と、
差分はあるが見た目は同じと判定した class / pie を、両バージョンで実際に描画して比較した。

| 図種 | 更新前 | 更新後 | 判定 |
| --- | --- | --- | --- |
| radar-beta | 右端の軸ラベル (`Power`) が図の縁で切れる。ラベルが軸線に寄る | ラベルが外側へ配置され切れない。上/左/右で anchor が使い分けられる | 改善 |
| block-beta | ボックスがラベル幅に対して狭く、文字が窮屈。`[( )]` の円柱が扁平でラベルが下端に寄る | ボックスがラベルに応じた適正サイズ。円柱の高さも正常 | 改善 |
| class | — | 完全に同一 (継承の白三角、集約の白菱形、コンポジションの黒菱形すべて同じ) | 変化なし |
| pie | — | 完全に同一 | 変化なし |

block-beta の矢印マーカーは 12 → 8 に小さくなる。図全体のバランスとしては違和感のない範囲だが、
**既存の block 図はレイアウトが変わる**点は認識しておく必要がある。

いずれも劣化方向の変化ではなく、mermaid 側でシェイプのサイズ計算とラベル配置が修正された結果である。
