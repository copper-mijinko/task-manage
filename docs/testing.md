...
| `electron/workspace.js` の `promoteLegacyMemos` / 移行 | `tests/unit/memo_to_node.test.ts` 10 件 | 旧メモをノードとして読むこと、保存で自分のディレクトリへ移ること、参照している画像をコピーして運ぶこと（元は残す）、まだ移行していないノードを削除しても親のディレクトリを消さないこと |
