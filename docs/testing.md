...
| `src/features/tasks/components/TreeTable.svelte`   | `toggles completion on the selected task with Ctrl+Enter` | 完了 / 未完了を同じ操作で往復できる | [tests/component/TreeTable.test.js](../tests/component/TreeTable.test.js) |
| `src/features/tasks/components/TreeTable.svelte`   | `Ctrl+Enter completes every task in a multi-selection` | 複数選択時は選択全件をまとめて完了にする | [tests/component/TreeTable.test.js](../tests/component/TreeTable.test.js) |
| `src/features/tasks/components/TreeTable.svelte`   | `Ctrl+Enter leaves the project root untouched` | プロジェクトルートは完了操作の対象外 | [tests/component/TreeTable.test.js](../tests/component/TreeTable.test.js) |
| `src/features/tasks/components/TreeTable.svelte`   | `collapses and expands a branch by toggling the row`                            | 折りたたみ操作で子行の表示と `closed_row_paths` を切り替える                                             | [tests/component/TreeTable.test.js](../tests/component/TreeTable.test.js) |
