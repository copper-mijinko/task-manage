...
`docs/` 配下の文書は、今時点の実装を断面として説明するためのものである。
課題の経緯、既知バグ、改善方針のような開発中メモは含めず、必要な場合はローカル管理の `開発メモ.md` に分けて扱う。

### 各ドキュメントの役割

| ファイル | 記載内容 |
| -------- | -------- |
| `specification.md` | 現在の機能仕様・画面構成・実装上の挙動 |
| `data.md` | データ保存先・構造・永続化・コンフリクト解決 |
| `architecture.md` | ソースコード階層・レイヤー責務・import 規約・主要パターン |
| `performance.md` | 起動時・ワークスペースプロジェクト選択時の読み込み最適化と遅延読み込み境界 |
| `testing.md` | テスト種別・テストケース一覧・実装状況・実行コマンド |
| `how-to-contribute.md` | 開発フロー・CI/リリース手順・認証認可・ドキュメント記載方針 |

### 記載すること

- 現在の実装として確定している仕様、構造、手順
- 現在動作しているコマンド、パス、ファイル名、API 名

### 記載しないこと

- 「v1.x で追加した」「v1.x 時点では」のような版ベースの経緯説明
- 変更の動機・背景の詳細（改版の概要を版表に一行で残すのは可）
- 今後の方針、検討中の改善、既知の引き継ぎ事項
- 課題・既知バグ・TODO

### フォーマット規約

- ファイルの 1 行目は `# タイトル`。メタデータ表は置かない
- タイトルの直後に `← [outline.md](outline.md)` の戻りリンクを置く（`README.md` と `outline.md` を除く）
- 末尾にはリンクを繰り返さない
- セクション区切りとして `---` 水平線を使わない
- ファイルパスは `docs/` からの相対パスで記述する（絶対パスや環境固有パスは使わない）
- 言語は日本語で統一する
