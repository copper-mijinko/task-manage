...
ブラウザ風の戻る・進むナビゲーションは `src/stores/navigation_history.ts` の `navigation_history` ストアで実現する。本アプリの「ページ」は `(selected_type, selected_id)` のタプルで定義される。

- **状態**: `{ entries: NavigationEntry[], index: number }`。`index === -1` は履歴なし。最大 100 件で、超過分は古い側から FIFO で落とす
- **エントリ形**: `{ selectedType, selectedId, projectDir }`。`WorkspaceProject` のとき記録時の `workspace_store.activeProjectDir` を併せて持つ。戻る時に `setActiveProject` を呼ばないと `loadWorkspaceData` が古い `activeProjectDir` を読んで「他プロジェクトのタスクを目当ての rootId で開こうとして失敗 → unknown ノード」になる。それを防ぐためにエントリに含める
- **記録経路**: `init()` で `selected_type` / `selected_id` 両方に `subscribe` を張る。subscriber は `Promise.resolve().then(...)` の microtask 1 回にコアレスし、`(selected_type, selected_id, projectDir)` の最新タプルを 1 エントリとして push する。これにより type と id を同期的に連続セットしても、中間タプルが履歴に積まれない
- **プログラム的 navigation の識別**: `back()` / `forward()` 内では `programmaticNavigation` フラグを true にしてから `selected_type` / `selected_id` を更新する。microtask が起動したときフラグが立っていれば push を skip し、フラグを下ろす。これによって戻る/進むそのものは履歴に再増殖しない
- **`activeProjectDir` の復元順序**: `WorkspaceProject` エントリを navigate する際は `workspace_store.setActiveProject(projectDir)` を **selected_type / selected_id より先に** 呼ぶ。`MenuList.selectWorkspaceProject()` と同じ順序を維持しないと、`selected_id` の subscriber (`loadWorkspaceData`) が古い `activeProjectDir` を読んでしまう
...
- **重複防止**: 直前の履歴ヘッドと同一タプル（type も id も等しい）であれば push しない。同じプロジェクトを連続クリックしても履歴は伸びない
- **空状態スキップ**: `(undefined, undefined)` は履歴に積まない（起動直後・プロジェクト削除直後のクリーンアップ状態）
- **サブウィンドウでの無効化**: `init_store()` の中で `window.location.hash === "#task-detail-window"` を判定し、タスク詳細サブウィンドウでは `navigation_history.init()` を呼ばない。サブウィンドウは独立した遷移空間ではないため
- **入力経路**: 
