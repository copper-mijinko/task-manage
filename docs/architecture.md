...
- **ページ内選択**: タスク行の選択（`table_selected_id`）はページ内の状態だが、戻ったときに TaskDetail / Memo のコンテキストを復元するためエントリに含める。**ページの主軸とは扱いを分け、同ページ内の選択切替は履歴を伸ばさず in-place 更新のみ行う**
- **エントリ形**: `{ selectedType, selectedId, projectDir, workspacePath, tableSelectedId }`
- **状態**: `{ entries: NavigationEntry[], index: number }`。`index === -1` は履歴なし。最大 100 件で、超過分は古い側から FIFO で落とす
- **記録経路**: `init()` で `selected_type` / `selected_id` / `table_selected_id` / `workspace_store` の各 store に `subscribe` を張る。subscriber は `Promise.resolve().then(...)` の microtask 1 回にコアレスし、最新タプルを 1 エントリとして push する。これにより同期的に連続セットしても、中間タプルが履歴に積まれない
- **page vs in-page の判定**: `commitRecord()` は `pageEqual()` でページ主軸の同一性を判定する。ページが同じなら entries[index] を tableSelectedId だけ差し替えて in-place 更新、ページが違えば forward 履歴を切り捨てて push する
- **着地待ち（`pendingNavigation`）**: `back()` / `forward()` 実行中は `pendingNavigation` に target エントリを置く。target 着地（pageEqual 一致）が確認できるまで `commitRecord` は entries を更新しない。これによって load 中に発生する `clearSelection()` → `selectOnly()` の transient な undefined 状態で target エントリの `tableSelectedId` を壊してしまうのを防ぐ。target 着地でクリア。ユーザが target を待たずに別ページへ動いた場合（pageEqual が崩れた場合）もクリアして通常記録に戻す
- **`pendingTaskDetailSelection` 経由の table 復元**: navigateTo は target の `tableSelectedId` を `setPendingTaskDetailSelection({ projectId, taskId, selectedType, projectDir })` に詰める。`loadProjectsData` / `loadWorkspaceData` がプロジェクト読み込み完了後にこのヒントを参照して `selectOnly(taskId)` を呼び、結果として entries[index].tableSelectedId と一致する状態に収束する
- **復元順序**: `setActive(workspacePath)` (async・fire-and-forget) → `setActiveProject(projectDir)` → `selected_type.set` → `selected_id.set`。`MenuList.selectWorkspaceProject()` と同じ順序にしておかないと、`selected_id` の subscriber (`loadWorkspaceData`) が古い `activeProjectDir` を読んでしまう
- **未消化記録の flush**: `back()` / `forward()` の冒頭で `flushPendingRecord()` を呼び、microtask 待ちの記録を同期的に消費してから navigation index を動かす。これによって「ユーザが直前のページに移った直後に戻るを連打」しても、移動前のページが履歴ヘッドにきちんと残る
...
- **対象外**: マルチ選択 (`selected_ids` / `selection_anchor_id`) と Inbox のアクティブアイテムは履歴に乗せない。前者はページ内かつ一時的な状態、後者は Inbox 単一ページ内のサブ選択として `InboxPanel` 自体が管理する
