...
- **記録経路**: `init()` で `selected_type` / `selected_id` 両方に `subscribe` を張る。subscriber は `Promise.resolve().then(...)` の microtask 1 回にコアレスし、`(selected_type, selected_id)` の最新タプルを 1 エントリとして push する。これにより type と id を同期的に連続セットしても、中間タプルが履歴に積まれない
