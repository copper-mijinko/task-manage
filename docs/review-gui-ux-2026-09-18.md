# GUI / UX 点検 (2026-09-18)

## 対象と方法

`claude/gui-ux-design-review-tuck9z` 時点の renderer を対象に、実 Electron
`BrowserWindow` (CDP `127.0.0.1:9222`) へ Playwright を接続して確認した。ソース
読解だけの指摘は含めず、すべて実行中のレンダラー上で再現・計測している。

判断基準として次の原則を用いた。

- **Nielsen のユーザビリティ 10 原則**（以下 N1〜N10）
- **WCAG 2.2**（達成基準番号で記載）
- **WAI-ARIA Authoring Practices (APG)** の `treegrid` / `menu` パターン
- **Fitts の法則**（ターゲット寸法と到達コスト）
- **Material Design 3** — `public/global.css` が自ら「M3-aligned」と宣言しており、
  そのトークン体系に対する内部整合性を基準にした

P1 は「ユーザーが機能を失う／誤った状態を信じさせられる」問題、P2 は
「操作効率・アクセシビリティ・一貫性の問題」とする。

---

## P1

### 1. 「戻る」「進む」が機能せず、かつ状態表示が誤っている

`src/stores/navigation_history.ts` / `src/features/navigation/components/Header.svelte:170-215`

実測（実 Electron、リロード直後）:

| 操作 | 表示中のページ | 戻るボタン | 進むボタン |
| --- | --- | --- | --- |
| 起動直後（未遷移） | `ws-demo` | **有効** | 無効 |
| Inbox へ遷移 | `Inbox` | 有効 | 無効 |
| 「戻る」をクリック | **`Inbox` のまま** | 有効 | **無効のまま** |

- 一度も遷移していない起動直後から「戻る」が有効になっている。
- 遷移後に「戻る」を押してもページが戻らず、「進む」も有効化されない。

`canGoBack` は `index > 0` で導出されるため、初期化時に積まれるエントリの持ち方と
`navigateTo()` の着地判定が噛み合っていない。ブラウザ風の戻る/進むは N3（ユーザーの
自由と離脱口）そのものの UI であり、それが無反応かつ「押せます」と表示している状態は
N1（システム状態の可視性）の違反でもある。

### 2. ウィンドウ幅 820px 未満で、ウィンドウ操作ボタンが画面外に出る

`src/features/navigation/components/Header.svelte:436-505` / `electron/window-state.js:16`

ウィンドウは `frame: false`（`electron/index.js:277`）で、最小化・最大化・閉じるは
ヘッダー右端の自前ボタンだけが提供する。そのヘッダーが幅に応じて縮まない。

実測（`Emulation.setDeviceMetricsOverride`、右端座標はビューポート幅との比較）:

| ウィンドウ幅 | 設定 | 最小化 | 最大化 | 閉じる |
| --- | --- | --- | --- | --- |
| 900px | 可視 | 可視 | 可視 | 可視 |
| 760px | 可視 | 可視 | **right=787（画面外）** | **right=817（画面外）** |
| 700px（`minWidth`） | **right=711（画面外）** | **right=757** | **right=787** | **right=817** |

`minWidth: 700` なので、この幅はユーザーが普通にリサイズして到達できる。ヘッダーは
`overflow` がスクロールしないため、はみ出したボタンは取り戻せない。**アプリを閉じる
手段が UI から消える**。N3 と N5（エラーの防止）の違反。

なお `Header.svelte:882` の `@media (max-width: 700px)` が唯一の折り返し規則だが、
`minWidth` が 700 のため実質 1px 幅のバンドでしか発火せず、緩和策として機能していない。

### 3. Inbox / 予定ボタンの選択状態が、永久に「未選択」を返す

`src/features/navigation/components/Header.svelte:307, 341`

```svelte
class:Active={$selected_type === "Inbox"}
aria-pressed={$selected_type === "Inbox"}
```

しかしクリックハンドラ（同 118-127 行）は `selected_type` に `"WorkspaceProject"`
を代入し、識別はセンチネル `selected_id` 側で行う。`selected_type` に `"Inbox"` /
`"Agenda"` を代入する箇所はソース全体に存在しない。

実測（Inbox ビューを開いた状態）:

```
ON INBOX VIEW: {"heading":"Inbox Workspace","pressed":"false","hasActive":false}
```

トグルボタンが自分の状態を常に false と報告する。N1 の違反であり、`aria-pressed` が
実態と食い違うため WCAG 4.1.2 (Name, Role, Value) にも抵触する。

### 4. `html { font-size: 75% }` がデザイントークン体系とユーザー設定の両方を壊している

`src/App.svelte:650`、`src/TaskDetailApp.svelte:294`

実測: `getComputedStyle(document.documentElement).fontSize === "12px"`。

影響は 2 つある。

**(a) px トークンと rem 指定が別スケールで乖離している。**
`global.css` の `--sp*` / `--font-*` / `--col-min-*` は px 固定なので等倍のまま、
コンポーネント側の `rem` 指定だけが 0.75 倍される。設計意図と実寸が全面的にずれる:

| 指定 | 意図 | 実測 |
| --- | --- | --- |
| `.InboxBtn { width: 1.75rem }` | 28px | **21px** |
| `.ExpandButton { width: 1rem }` | 16px | **12px** |
| `--card-header-min-h: 2.25rem` | 36px | 27px |
| `aside.Sidebar { width: 18rem }` | 288px | 216px |

`global.css` のコメントは「`--card-header-min-h: 2rem` を IconButton の 2rem に合わせる」
のように rem を実 px として説明しており、前提が成立していない。M3 の 4pt グリッドを
標榜する以上、この二重スケールは土台の不整合にあたる。

**(b) ユーザーのフォントサイズ設定が一律 25% 縮む。**
`75%` は利用者の既定サイズに対する相対値なので拡大操作自体は効く（WCAG 1.4.4 の
「200% まで拡大できること」は満たす）。ただし OS / Chromium 側で既定フォントを
大きくしている利用者でも、常にその 0.75 倍から始まる。読みやすさのために設定を
上げている人ほど、アプリ側で一律に打ち消される度合いが大きい。

---

## P2

### 5. クリックターゲットが広範に 24px 未満（実測 39 箇所）

WCAG 2.2 SC 2.5.8 (Target Size, Minimum) は 24×24 CSS px を最低線とする。実測での
主な違反:

| 要素 | 実測 |
| --- | --- |
| 行の展開／折りたたみ `.ExpandButton` | **12 × 12** |
| 全選択チェックボックス `.HeaderCheckbox` | **11 × 11** |
| 列ソートボタン（6 列分） | 18 × 18 |
| 列フィルターボタン（6 列分） | 高さ 15 |
| 戻る／進む／Inbox／設定 | 21 × 21 |
| 行メニュー `.menu-button` | 21 × 21 |
| ステータス／日付セル | 高さ 23 |
| ペイン分割ハンドル `.Resizer` | 幅 **5** |

ツリー UI で最も反復操作される展開トグルが 12px 角なのは、Fitts の法則上もっとも
コストの高い配置になっている。原因の多くは P1-4 の 0.75 倍縮小である。

### 6. treegrid の roving tabindex が行内の子要素に破られ、文書全体で 90 タブストップ

`src/features/tasks/components/TreeTableRow.svelte`

行 (`role="row"`) 自体は `tabindex="-1" / "0"` で roving tabindex を実装しているが、
行内の操作要素は素の `tabindex` のまま（実測）:

```
row1 tabindex=-1 children=[DIV:-1, BUTTON:null, INPUT:null, BUTTON:null, BUTTON:null, BUTTON:null, BUTTON:null]
```

結果、6 行しかない状態で文書内のタブストップが **90** ある。APG の treegrid は
「グリッド全体で 1 タブストップ、内部は矢印キー」を要求しており、行あたり 5〜6 個の
タブストップが残っているため roving tabindex が無効化されている。実データ規模では
ツリーを通過するだけで数百回の Tab が必要になる。

加えて、ツールバー 10 個 + 列ヘッダー 13 個の計 23 タブストップを越えないと
最初のデータ行に到達しない。

### 7. 確認ダイアログのボタンが英語のまま、破壊的操作にも既定スタイル

`src/lib/primitives/Dialog.svelte:9-10`

```ts
export let ok = "ok";
export let cancel = "cancel";
```

全 6 箇所の呼び出し（`MenuList.svelte:695,702` プロジェクト削除 /
`TaskDetail.svelte:946,953` / `TreeTable.svelte:1641` / `MainPage.svelte:1332`）が
この既定値をそのまま使っている。実測スクリーンショットでも、日本語の
「アーカイブの確認」ダイアログに **`cancel` / `ok`** が並ぶ。

さらに `MainPage.svelte` には英語のまま残った文字列がある:

- `1356`: `header="Alert."`
- `249`: `let alert_content = "Cannot delete the root node.";`

問題は 3 点。

1. 言語の不統一（N2: 現実世界との一致）。
2. ボタンラベルが動作を述べていない。破壊的確認は「アーカイブする」「完全に削除」と
   名詞化するのが定石で、`ok` は何が起きるか伝えない。
3. 完全削除を含む破壊的操作でも `ok` が `variant="filled"`（通常のプライマリ青）で、
   危険度が色に反映されない。N5（エラーの防止）。

また実測ではダイアログを開いた直後のフォーカスが**閉じる (×) ボタン**に当たる。
破壊的確認では安全側（キャンセル）かダイアログ本体に置くべき。

### 8. ホバー／選択状態が視覚的に no-op になっている

`src/features/navigation/components/Header.svelte:593-596, 731-741`

ヘッダーは `--hover-bg: rgba(255,255,255,0.14)` を定義し、それを**通常時の背景色**に
使っている。そのうえでホバー時にも同じ変数を指定している:

```css
.NavHistoryBtn        { background-color: var(--hover-bg); }
.NavHistoryBtn:hover:not(:disabled) { background-color: var(--hover-bg); }  /* 同値 */
.InboxBtn             { background-color: var(--hover-bg); }
.InboxBtn:hover       { background-color: var(--hover-bg); }                /* 同値 */
.InboxBtn.Active      { background-color: var(--hover-bg);
                        box-shadow: inset 0 0 0 1px var(--hover-bg); }      /* 背景と同色の内枠 */
.SearchField:focus-within { background-color: var(--hover-bg); }            /* 同値 */
```

実測で `.NavHistoryBtn` の背景が `rgba(255, 255, 255, 0.14)` = `--hover-bg` と一致する
ことを確認した。ホバーは border-color だけが変わり、`.Active` はホバーと区別できず、
内側 box-shadow は背景と同色のため不可視。検索欄のフォーカスも背景が変わらない。
N1 の違反。

### 9. ヘッダーで「muted」が「default」に潰され、プレースホルダーと入力値が同色

`src/features/navigation/components/Header.svelte:531-532`

```css
--fg-default: var(--on-theme-text);
--fg-muted:   var(--on-theme-text);   /* 同じ値 */
```

ヘッダー配下の 8 箇所（`.SearchInput::placeholder`、`.SearchCount`、`.SearchShortcut`、
`.SaveIndicator`、`.WinCtrlBtn` ほか）が `--fg-muted` を参照しているため、階層が消える。

実測:

```
input_color:       rgb(244, 247, 255)
placeholder_color: rgb(244, 247, 255)   ← 同一
```

**プレースホルダーと実入力値が同じ色で描かれる**ため、検索欄が空なのか
「画面内をハイライト検索…」と入力済みなのかを見分けられない。N1 の違反。

### 10. 対象が存在しなくてもアーカイブ・元に戻す・やり直しが有効

`src/pages/MainPage.svelte`

リロード直後（操作履歴ゼロ・選択ゼロ）の実測:

| ボタン | disabled |
| --- | --- |
| 上に移動 / 下に移動 / インデント / アウトデント | `true`（正しい） |
| **元に戻す** | `false` |
| **やり直し** | `false` |
| **アーカイブ** | `false` |

押しても何も起きず、フィードバックもない。同じツールバーの中で無効化ロジックの
基準が揃っていないため、ユーザーは「無効化＝実行不可」という手がかりを信頼できなく
なる。N5、および N4（一貫性と標準）。

### 11. オーバーフローメニューの ARIA 構造が不正、トグル項目に状態がない

`src/features/tasks/components/TaskMenu.svelte:142-153`

実測した構造:

```
UL role=menu
  └ LI role=null          ← menu の直接の子が menuitem でない
      └ BUTTON role=menuitem
```

`role="menu"` が所有できるのは `menuitem` / `menuitemradio` / `menuitemcheckbox` /
`group` / `separator` に限られる。間に role のない `li` が挟まるため所有関係が切れ、
支援技術への項目数の伝達が壊れる（WCAG 4.1.2）。

加えて、表示トグル 3 項目（「ガントチャートを表示」「詳細欄を隠す」
「アーカイブ済みを表示」）はすべて `role="menuitem"` で `aria-checked` を持たない。
視覚的にもチェックマークがなく、ラベルの動詞が反転するだけなので、現在どちらの状態か
をメニューから読み取れない。`menuitemcheckbox` + `aria-checked` が適切。

### 12. 行追加後にフォーカスが移らず、全行が「新しいノード」になる

`src/pages/MainPage.svelte:855-874`

実測:

```
focus before add: BUTTON aria=タスク追加
focus after  add: BUTTON aria=タスク追加   ← 変わらない
```

追加された行の名前は既定文字列「新しいノード」のまま、名前入力にフォーカスが入らない。
3 回押すと同名の行が 3 つ並び、サイドバーにも「新しいノード」が 4 件並ぶ状態を実測した。
アウトライナ／ツリー系 UI では「追加 → 即入力」が基本動線であり、毎回マウスに持ち替え
させている。N7（柔軟性と効率性）。

### 13. サイドバーがスクリム付きモーダルドロワーで、現在地の表示もない

`src/App.svelte:512, 687-705`

デスクトップアプリ（`minWidth` 700、既定 1280px）にもかかわらず、プロジェクト
ナビゲーションは `position: absolute` + 背面スクリムのオーバーレイになっている。
実測スクリーンショット（1280px 幅）では、幅 216px のサイドバーを開くだけで残り
1064px のツリーと詳細ペインが暗転して操作不能になる。

- ツリーを見ながらプロジェクトを切り替える、ツリーへドラッグする、といった操作ができない。
- 開いている間、リストのどの項目が現在表示中かを示すハイライトがない（Inbox と 4 つの
  ノードが同一の見た目）。

常時表示できる幅があるのに一時的オーバーレイを使うのはモバイルパターンの流用であり、
N6（記憶より認識）と N1 の両方に効いている。

### 14. 面（サーフェス）と整列の一貫性が崩れている

- **詳細ペインの空状態にだけ Card がない。** 実測で、空状態の `Pane` の背景は
  `rgba(0,0,0,0)` で、親 `.Content` の `rgb(217,222,225)` がそのまま見える。左のツリーは
  白い Card（枠線・角丸つき）なので、右半分だけ「抜けた穴」に見える。
- **列ヘッダーは中央揃え、セルは左揃え。** 実測で `[role="columnheader"]` は
  `justify-content: center`。タスク名列（x=37..410）ではヘッダー「タスク名」が x≈200 に
  中央寄せされる一方、セルのテキストは x≈85 から始まる。列見出しと内容の視覚的な
  対応（ゲシュタルトの近接）が切れ、幅の広い列ほどずれが大きい。
- **本文の編集アフォーダンスが形式で変わる。** `TaskDetail.svelte:890` で
  「編集／プレビュー」トグルは `bodyFormat !== "markdown"` のときだけ出る。

N4（一貫性と標準）。

### 15. 行番号列が選択行でチェックボックスに化け、序数が飛ぶ

`src/features/tasks/components/TreeTableRow.svelte`

実測した行テキスト（3 行目を選択中）:

```
row1 "1/—/—/—/—"
row2 "2/未着手/—/—/—"
row3 "—/—/—/—"        ← 番号が消えている
row4 "4/—/—/—/—"
row5 "5/—/—/—/—"
```

同じ列が状況によって「行番号」と「選択チェックボックス」の 2 つの意味を持つため、
選択中は連番が 2 → （空白）→ 4 と飛ぶ。番号を手がかりに行を指し示すことができない。
選択という主要機能がホバー／選択時にしか現れない点も、発見可能性の問題（N6）。

### 16. 保存モデルが二重に提示されている

`src/features/tasks/components/TaskDetail.svelte:900-908`

ヘッダーは常時「保存済み」インジケータ（自動保存）を出しているのに、詳細ペインには
フロッピーディスク図像の「今すぐ保存」ボタン（`memoEditor?.flush()`）がある。
自動保存を謳う UI に手動保存ボタンが併存すると、「押さないと保存されないのでは」と
いう疑いを生み、自動保存の信頼を毀損する。N1 と N4。

あわせて、詳細ペインの読み取り専用フィールドが `<label class="detail-field">` で
マークアップされている（実測）。対応するフォームコントロールを持たない `label` は
支援技術にとって無意味な関連付けになる。

---

## 良かった点

指摘の公平を期すために記録する。

- **文字コントラストは実測で違反ゼロ。** レンダリング後の合成色で全テキストノードを
  走査したが、WCAG AA (4.5:1 / 大文字 3:1) を下回るものは 0 件だった。
  `theme.ts` の `Primary.text` / `Error.text` / `Warning.text` のように「塗り用」と
  「文字用」を分けた設計が効いている。
- `@media (prefers-reduced-motion: reduce)` を全域で尊重している。
- `:focus-visible` のフォーカスリングが全体で統一されている。
- 行 (`role="row"`) 側の roving tabindex、`aria-level`、`aria-selected`、
  `role="treegrid"` など、ツリーのセマンティクスの骨格自体は入っている（6 を直せば
  活きる）。
- オーバーフローメニューには `role="separator"` によるグルーピングがある。

---

## 対応状況 (2026-09-18 追記)

上記のうち、実 Electron 上で修正と再計測まで済ませたものを記録する。判定はすべて
CDP 接続したレンダラーでの実測による。

| # | 項目 | 対応 | 実測での確認 |
| --- | --- | --- | --- |
| P1-1 | 戻る／進むが機能しない | 修正 | 起動直後は両方無効。Inbox へ遷移 → 戻る → ws-demo に戻り「進む」が有効化、進む → Inbox に復帰 |
| P1-2 | 幅 820px 未満でウィンドウ操作ボタンが画面外 | 修正 | 1280 / 1000 / 880 / 800 / 700px のすべてで画面外要素 0 件 |
| P1-3 | Inbox ボタンの選択状態が常に false | 修正 | Inbox 表示中に `aria-pressed="true"`、離脱で `false` |
| P1-4 | `html { font-size: 75% }` | **見送り** | 下記「見送った項目」参照 |
| P2-5 | 24px 未満のクリックターゲット | 修正 | **39 件 → 1 件**（残りは意図的な 11px の分割ハンドル） |
| P2-6 | treegrid のタブストップ | **見送り** | 下記参照 |
| P2-7 | 確認ダイアログの `ok` / `cancel` | 修正 | 「キャンセル」/「アーカイブする」「完全に削除」「削除する」「変換する」。破壊的操作は確定ボタンがエラー色 |
| P2-8 | ホバー／選択状態が no-op | 修正 | 静止 `rgba(255,255,255,0.1)` / ホバー `0.24` / 選択 `0.34` + アクセント枠 |
| P2-9 | プレースホルダーと入力値が同色 | 修正 | 入力値 `#f4f7ff` / プレースホルダー `#b6c4e4`（濃紺に対し 6.07:1） |
| P2-10 | 対象なしでもアーカイブが有効 | 一部修正 | アーカイブは `disabled=true` に。元に戻す／やり直しは見送り（下記） |
| P2-11 | メニューの ARIA 構造・トグル状態 | 修正 | `li[role=none] > button[role=menuitem]`、表示トグルは `menuitemcheckbox` + `aria-checked` + 視覚的チェック |
| P2-12 | 追加後にフォーカスが移らない | 修正 | 追加直後に新しい行の名前入力へフォーカス、既定名を全選択（`sel=0-6`）。そのまま入力できる |
| P2-13〜16 | サイドバー、面と整列、行番号列、保存モデル | **見送り** | 下記参照 |

あわせて、点検時には出ていなかった状態（検索パネルを開いた状態）で新たに見つかった
コントラスト違反 2 件も直した。`PageSearchBox` の `.search-button` / `.result-count`
が `Primary-main` を使って 4.09:1 だったもので、`theme.ts` が同じ理由で用意している
`Primary-text` に寄せて 5.11:1（Light）/ 7.98:1（Dark）にした。

### 検証

- `svelte-check` 0 errors / 0 warnings、`eslint` 0、`prettier --check` 通過。
- `vitest run` 873 passed / 7 skipped（既存スキップ）。
  - `navigation_history` に回帰テストを 2 件追加（センチネルを積まないこと、
    `selectedType` だけ決まった途中経過を積まないこと）。
  - ラベルとロールの変更に追随して component テスト 4 件、E2E の確認ボタン参照
    2 箇所を更新。
- コントラスト実測: AA 違反 0 件（修正前後とも）。
- 実 Electron でのコンソールエラー: なし。

### 見送った項目と理由

**P1-4 `html { font-size: 75% }`。** これを外すと rem 指定のすべてが 1.33 倍になる
一方 px トークンは変わらないため、全画面のレイアウトが一斉に動く。Gantt / Finder /
Graph / メモエディタ / 添付など今回走査していない領域まで巻き込むので、px トークンの
再調整とデザイン側の確認とセットで行うべきと判断した。代わりに、この二重スケールが
実害として出ていた「24px 未満のターゲット」を px の `--tap-min` トークンで個別に
解消した（P2-5）。根本原因は残っている。

**P2-6 treegrid のタブストップ。** 行内の操作要素を `tabindex="-1"` にするには、
セル間を矢印キーで移動する実装を同時に入れないとキーボードから操作できなくなる。
APG 準拠のセルナビゲーションは独立した変更として扱うべき規模。

**P2-10 のうち元に戻す／やり直し。** 履歴の深さは main プロセスの
`graph-v1.json` の `undo` / `redo` 配列にあり、レンダラーへは `graph` しか
渡っていない。正しく有効・無効を出すには IPC のペイロードに履歴深さを足す必要が
あり、main プロセスと 4 つの呼び出し経路に及ぶ。今回は「押せるのに何も起きない」
度合いがより重い破壊的操作（アーカイブ）を先に直した。

**P2-13〜16。** サイドバーを常時表示のレールにする、詳細ペインの空状態に Card を
与える、列ヘッダーの揃えを変える、行番号列と選択チェックボックスを分ける、
自動保存と手動保存の関係を整理する — いずれも見た目と情報設計の方針決定を伴う。
バグ修正ではなくデザインの選択なので、判断を仰ぐべきと考えて手を付けていない。

## 見送り分の再点検とメモエディタ (2026-09-18 追記2)

見送っていた項目を再検討し、走査対象外だったメモエディタ（Markdown / Quill）を
実 Electron 上で点検した。

### 見送り分の結果

| # | 項目 | 結果 |
| --- | --- | --- |
| P2-10 残り | 元に戻す／やり直しの有効・無効 | **修正** |
| P2-6 | treegrid のタブストップ | **修正** |
| P1-4 | `html { font-size: 75% }` | 据え置き（判断と根拠を下記に記録） |
| P2-13〜16 | サイドバー・面と整列・行番号列・保存モデル | 未了（方針の確認待ち） |

**P2-10（元に戻す／やり直し）。** main プロセスの `graph-v1.json` が持つ
`undo` / `redo` の段数を、読み出し経路でだけ graph に添えてレンダラーへ渡すように
した（`withHistoryDepth`）。`document.graph` 自体には足さないので、ディスクには
書かれない。legacy 経路は自前のスタックを `canUndoLegacy` / `canRedoLegacy` として
公開した。実測で、元に戻すを 13 回押して履歴が尽きた時点でボタンがちょうど
無効化されることを確認している。

**P2-6（タブストップ）。** 行の矢印キー移動は既に実装されていた
（`TreeTableRow.handleKeydown` → `TreeTable`）ので、キー割り当ては一切変えずに、
行の中のコントロールを「いま操作している行だけ」停留点にした
（`cellTabIndex = isTabStop ? 0 : -1`）。

| | 修正前 | 修正後 |
| --- | --- | --- |
| 6〜7 行のときの文書全体のタブストップ | 90 | 71 |
| 1 行増えるごとの増加 | +5〜6 | **±0** |

Tab でツリーに入る → その行の 5 つの操作要素を辿る → Tab で抜ける。矢印キーで
行を移ると、その行の操作要素が辿れるようになる。編集経路は変えていない。

**P1-4（`font-size: 75%`）は据え置きとした。** 点検時に書いた
「ユーザーのフォント設定が 25% 縮む」という指摘は、このアプリには
アプリ内の表示倍率（Ctrl +/-/0、Ctrl+ホイール、設定モーダルの「表示倍率」、
`meta.json` に永続化）があり、こちらは px と rem を同じ比率で拡大するので
逃げ道としては十分に機能する。残るのは「rem 指定と px トークンが別スケール」
という内部の不整合で、その実害だった 24px 未満のターゲットは
px トークン `--tap-min` で解消済み。75% を外すと rem 指定だけが 1.33 倍になり
全画面が動くため、px トークンの再調整とセットの独立した変更として扱うべきと
判断した。同じ罠を繰り返さないよう、宣言箇所（`App.svelte` /
`TaskDetailApp.svelte`）に理由と「最小寸法には rem ではなく `--tap-min` を使う」
という指針をコメントで残した。

### メモエディタ (Markdown / CodeMirror)

| 指摘 | 対応 |
| --- | --- |
| 空の本文が不活性で、編集に入る唯一の経路が 24px のドロップダウンだった | **修正** |
| 構文ハイライトが AA 未満（強調 1.72:1 / コード 3.43:1 / リスト・リンク 4.37:1） | **修正** |
| エディター／プレビューの分割ハンドルが 5px | **修正**（11px） |
| プレビューのチェックボックスが 13×13 なのに実操作 | **修正**（24×24） |
| 「保存要求中」「保存要求済み」という実装側の語彙 | **修正**（ヘッダーと同じ「保存中...」「保存済み」に統一） |
| 表示モードの combobox に `aria-controls` / listbox の id が無い | **修正** |
| 全角・半角スペースをすべて `·` で可視化している | 未了（方針の確認待ち） |

**空の本文に入れなかった件がいちばん重い。** 実測で、空メモの本文をクリック
してもダブルクリックしてもフォーカスは `<body>` のままで、エディタは現れなかった。
画面でいちばん大きな面（480×584）が不活性なまま、いちばん小さいコントロールが
唯一の入口になっていた。本文が空のときはプレビュー面自体を
`role="button"` / `tabindex="0"` の入口にし、クリックでも Enter でも編集に入る
ようにした（中身があるときは従来どおり。プレビュー内の選択やリンク操作を
奪わないため）。

**構文ハイライトのコントラスト。** エディタ背景は `Main-light`。ライトテーマでは
`#F9F9F9` で、そこに載る色は実測で strong / emphasis が `#FBA9C7` = **1.72:1**、
monospace が `#FF2E63` = 3.43:1、list / link が `#1976d2` = 4.37:1 だった。
**強調した文字がいちばん読めない**という逆転が起きていた。`theme.ts` が
同じ理由で用意している `*-text` の規約に合わせ、`Accent.text` を追加して
（Light `#C2184A` = 5.66:1 / Dark `#FBA9C7` = 5.66:1）そちらを参照するようにした。

### 検証

- `svelte-check` 0 / `eslint` 0 / `prettier --check` 通過。
- `vitest run` 873 passed / 7 skipped。
- **`playwright test` 30 passed。** E2E は `npm test` に含まれないため前回の
  変更（メニューのロール変更・ダイアログのラベル変更）が反映されておらず、
  3 件が落ちていた。今回そこまで含めて追随させた。今後この種の変更をするときは
  E2E も併せて回すこと。

## 第 3 巡 — 方針確認後の対応 (2026-09-18 追記3)

判断を仰いでいた項目について方針が決まったので、そのとおりに実装した。

| 項目 | 方針 | 結果 |
| --- | --- | --- |
| `html { font-size: 75% }` | 専用の変更として着手 | **完了**（レンダリングは画素単位で不変） |
| サイドバー | 広い幅では常時表示のレール | **完了**（1000px 以上でレール、未満はオーバーレイ） |
| 詳細ペインの空状態に Card | 着手 | **完了** |
| 列ヘッダーの揃えをセルに合わせる | 着手 | **完了**（3px のずれも併せて解消） |
| 保存モデルの一本化 | 着手 | **完了** |
| メモの空白可視化 | 今のまま維持 | 変更なし |
| 行番号列と選択チェックボックスの分離 | 選択されず | 変更なし |

### rem スケールの統一

`rem` 指定を一律 0.75 倍（55 ファイル・552 箇所）、px トークンを 16px 基準の rem
へ変換、ルートの 75% を削除した。**変換が中立であることを仮定せず実測で確認**して
いる。700 / 1000 / 1280px のスクリーンショットが変更前後で画素単位で一致
（各幅で差分 0 画素）、幾何も完全一致（行高 32、ツリーペイン 758×751、
アイコンボタン 24×24、タスク名列 373、サイドバー 216）。

変わるのは意図した挙動のほうだけ。既定フォント 20px では全体が同じ比率で拡大し
（`--sp4` 20px、アイコンボタン 30、行 40）、12px では縮むが `--tap-min` は
`max(1.5rem, 24px)` で 24px を保つ。以前は余白が設定を完全に無視し、寸法だけが
0.75 倍で追従していた。

### サイドバー

1000px 以上では `position: relative` のレールとして本文と併置し、スクリムを出さない。
実測（1280px）で、開くとツリーが `16..758` → `232..844` へ寄り、スクリムなしで
全体が操作可能。900px では従来どおりオーバーレイで、ツリーの幾何は `16..530` のまま
変わらずスクリムが出る。

### 列ヘッダーの揃え

見出しを左寄せにし、セル側 (`.TableData`) と同じ横 padding に揃えた。あわせて、
先頭のチェックボックス列が**ヘッダー 21px / 行 24px** で食い違っており、以降の
すべての列が見出しに対して 3px ずれていたのを直した（第 1 巡で行側だけを 24px に
広げた際の取りこぼし）。実測で見出しとセルの箱が一致
（`40..410` / `410..506` / `506..602` …）。

### 保存モデル

画面から保存に関する表示・操作が 3 つ消えて 1 つになった。メモの保存状態を
ヘッダーと同じ `saveStatus` ストアへ流し、メモ内の保存表示と、フロッピー図像の
「今すぐ保存」ボタンを削除した。手動保存は不要で、下書きを失いうる操作
（形式変換・削除・ページ遷移など）の前には `TaskDetail` が `memoEditor.flush()`
を呼んでいる。実測で、メモに入力するとヘッダーが
`保存済み` → `保存待ち` → `保存済み` と動くことを確認した（以前はメモ編集では
まったく動かなかった）。

### 検証で見つかったこと

`npx playwright test` は `renderer/` のビルド済み出力を見るので、**ビルドし直さずに
回すと古いコードを検査してしまう**。第 3 巡の途中で一度これに気づかず
「30 passed」を得ていた。ビルド後に回し直したところ 2 件落ちており、いずれも
今回の変更に追随していないテストだった（廃止した「今すぐ保存」ボタンへの依存、
オーバーレイ前提のサイドバー assertion）。E2E を回すときは
`npm run test:e2e`（build を含む）を使うこと。

最終状態: `svelte-check` 0 / `eslint` 0 / `prettier` 通過 /
`vitest` 873 passed / `playwright` 30 passed（ビルド後）。

## 推奨する着手順

1. **P1-4（`font-size: 75%`）を最初に外す。** これは 5（ターゲット寸法）の大半と、
   トークン体系のコメントと実寸の乖離を同時に解消する。ただし全画面の寸法が変わるため、
   px トークン側の再調整とセットで行う必要がある。
2. **P1-2（ウィンドウ操作ボタンの消失）** — 機能喪失であり、修正は局所的。
3. **P1-1 / P1-3** — 状態表示の嘘を止める。
4. P2-7 のダイアログ文言、P2-8 / P2-9 の no-op な状態表現は、いずれも小さい変更で
   体感が大きく変わる。

なお本点検は視覚・操作・アクセシビリティの観点に限っており、Gantt、Finder、Graph
ビュー、メモエディタ（Quill / Markdown）、添付・画像まわりは今回の走査対象に含めて
いない。

---

# 第 4 巡（細部の一貫性）

第 3 巡のあと、ツリーグリッド以外にも残っていた「見た目が揃っていない」
「主従が逆」「重なる」箇所を洗い出し、依頼を受けた 7 件を修正した。
採番は洗い出し時の通し番号のまま残す（②と⑧は本人判断で見送り）。

## 修正したもの

### ① ツールバーの表現がボタンごとに違う（一貫性 / Nielsen #4）

塗り・枠線・素のアイコンが混在し、「追加」だけが青い塗り、「子タスク追加」だけが
枠線という状態だった。強調のレベルが操作の重要度ではなく実装の都合で決まっていた。

- 追加 / 子タスク追加を `variant="text"` に統一し、色分けだけで役割を示す。
- 破壊的操作（アーカイブ／完全削除、復元）を区切り線を挟んだ独立グループへ移し、
  「追加」の真横に並ばないようにした。
- アイコンの `stroke-width` が 1.8 と 2 で混在していたので 2 に統一。

結果のツールバー: `＋ ↳＋ │ 🗑 │ ↑ ↓ ⇐ ⇒ │ ↺ ↻ │ ⋯`

### ③ 「所属する場所」が読めない（近接 / Gestalt、Nielsen #8）

リンク・パス・状態ラベル・メニューが素の空白だけで横一列に並び、どこまでが 1 件か、
どれが押せるのかが読めなかった。

- 1 件 = 1 グリッド行に変更。`grid-template-columns: auto 1fr auto` で
  「名前 / 状態チップ / ⋯ メニュー」を 1 行目、経路を 2 行目に置き、下線で区切る。
- 押せる親名に `text-decoration: underline` を付け、当たり判定を `--tap-min` に。
- 「現在の表示経路」を背景付きのチップにして、名前の一部に見えないようにした。
- 節直下の text ボタン（「所属先を追加」）が Button primitive の左 padding 16px を
  持っていて行頭がずれていたので `padding-left: 0` に。

> 実装中の落とし穴: 最初 `1fr auto` の 2 列にしたところ、列を明示していない
> チップが暗黙の 3 列目に置かれ、⋯ メニューが中央・チップが右端に飛んだ。
> 3 要素それぞれに `grid-column` を明示して解決した。

### ④ ステータス候補の「ステータスなし」だけ字下げがずれる（整列）

色ドットを `display: none` にしていたため、その行だけラベルが左に寄っていた。
`.s-dot-static[data-status=""]` を `visibility: hidden` に変え、8px の枠は残す。

### ⑤ フィルターのチェックボックスだけ小さい（一貫性）

11px でツリーグリッドの 14px と揃っていなかった。`var(--font-body-md)` に統一。

（前回「『なし』だけドットが無い」「当たり判定も 11px」と書いたのは誤りだった。
実測するとラベル位置は全行 x=968 で揃っており、行全体が 210×26 の `<label>` なので
当たり判定は基準を満たしている。実在した問題はグリフ寸法だけ。）

### ⑥ 設定の拡大縮小行だけ配置が崩れる（整列）

`.Field` が `justify-content: space-between` なのに子が 4 つあったため、
ボタン 3 つが行全体に散っていた。3 つを `.FieldControls` でまとめて子を 2 つにし、
上の「標準 / コンパクト」行と右端が揃うようにした。

### ⑦ クイック追加の主ボタンが逆（Nielsen #6 / 主従）

繰り返し使う「追加」(Enter) が素のボタンで、たまにしか使わない「追加して閉じる」
(Shift+Enter) が青い主ボタンだった。主従を入れ替え、プレースホルダも
「思いついたタスクを入力」に短くした。

### ⑨ ツールチップがメニューの上に残る（Nielsen #1）

ホバーで出たツールチップが、そのままメニューを開いても消えず項目に重なっていた。
`dismissAllTooltips()` を `@lib/actions` に追加し、`TaskName` と `MainPage` の
メニューを開く時点で呼ぶ。実測: ホバー時 `.__tooltip-element` 1 件 →
メニューを開いた直後 0 件。

## 見送ったもの

- **② 並べ替えの矢印位置** — 見出しを左揃えにしたことで矢印が次の列の見出しに
  近づいたが、本人判断で現状維持。
- **⑧ ガントの列が詰まる** — 詳細ペインを畳めば名前列は 180px → 377px に広がる。
  手動で畳む／列を減らせば済むため、問題ではないと判断された。

## 検証

`svelte-check` 0 errors 0 warnings / `eslint` 0 / `prettier` 通過 /
`vitest` 873 passed 7 skipped / `playwright` 30 passed（`npm run test:e2e`）。
GUI は Electron CDP 越しの Playwright で実機確認し、コンソールエラー 0 件。
