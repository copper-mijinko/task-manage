<script>
  ...
  const createResizers = (resizers=[], is_default=true, resize_observer=null) => {
    ...
    // Set width
    const default_ratio_sum = $tree_data.headers.reduce((partialSum, header) => partialSum + header.default_ratio, 0);
    const default_root_width = rows[0].getBoundingClientRect().width;
    const default_data_widths = $tree_data.headers.map((header) => default_root_width * header.default_ratio / default_ratio_sum);
    ...
    resize_observer = new ResizeObserver((entries) => {
      ...
      const new_header_widths = headers.map((h) => (h.getBoundingClientRect().width * new_table_width / table_width));
      headers.forEach((header, index) => {
    ...
    // Create resizers and their events
      ...
      data_rows.forEach((rows, _) => {
        ...
        datas_r.push(rows[i+1])
			...
			const min_w = parseFloat(window.getComputedStyle(header).minWidth, 10) || 10;
			const min_wr = parseFloat(window.getComputedStyle(header_r).minWidth, 10) || 10;
...
<div bind:this={table_root}	class:TableRoot={true} style="--minWidth: {minWidth}">
...
<style>
	...
	.TableRoot :global(.HandlingResizer), .TableRoot :global(.Resizer:hover) {
